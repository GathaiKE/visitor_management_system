...
@Component({
  selector: 'app-active',
  standalone: true,
  imports: [PaginatorComponent, HeaderComponent, SidenavComponent, SpinnerComponent, FormComponent, NgFor, FormsModule],
...
fetchData(){
  ...
  this.adminsService.admins$.subscribe(res=>{
    if(res){
      this.admins = res.filter(admin=>admin.is_admin && admin.is_active)
      this.unfilteredAdmins = res.filter(admin=>admin.is_admin && admin.is_active)
